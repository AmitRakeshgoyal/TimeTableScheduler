...
while ($row = mysqli_fetch_assoc($query)) {
    ...
    if (isset($temp->subjectteacher)) {
        ...
        $row1 = mysqli_fetch_assoc($teacheralias_query);
        $temp->alias = $row1['alias'];
    }
    $subjects[$count++] = $temp;
}
...
for ($I = 0; $I < ($subjects_count * 5); $I++) {
    ...
    $classroom_query = mysqli_query(mysqli_connect("localhost", "root", "", "ttms"),
        "SELECT name FROM classrooms WHERE status='$sem'");
...
while ($rows = mysqli_fetch_assoc($query)) {
    if (!($rows['course_type'] == 'LAB'))
        continue;
    $temp = new Subject();
    $temp->code = $rows['subject_code'];
    $temp->semester = $rows['semester'];
    $temp->subjectteacher = $rows['allotedto'];
    $temp->subjectteacher2 = $rows['allotedto2'];
    $temp->subjectteacher3 = $rows['allotedto3'];
    $teacheralias_query = mysqli_query(mysqli_connect("localhost", "root", "", "ttms"),
        "SELECT * FROM teachers WHERE faculty_number='$temp->subjectteacher'");
    $row1 = mysqli_fetch_assoc($teacheralias_query);
    $temp->alias = $row1['alias'];
    $teacheralias_query2 = mysqli_query(mysqli_connect("localhost", "root", "", "ttms"),
        "SELECT * FROM teachers WHERE faculty_number='$temp->subjectteacher2'");
    $row2 = mysqli_fetch_assoc($teacheralias_query2);
    $temp->alias2 = $row2['alias'];
    $teacheralias_query3 = mysqli_query(mysqli_connect("localhost", "root", "", "ttms"),
        "SELECT * FROM teachers WHERE faculty_number='$temp->subjectteacher3'");
    $row3 = mysqli_fetch_assoc($teacheralias_query3);
    $temp->alias3 = $row3['alias'];
